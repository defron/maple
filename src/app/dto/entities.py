...
class User(Base):
    ...
    pass_hash: Mapped[str] = mapped_column(CHAR(length=128), nullable=False)
    salt: Mapped[str] = mapped_column(CHAR(length=64), nullable=False)
...
class Category(Base):
    ...
    is_cashflow: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default=text("TRUE"))
...
class Transaction(Base):
    ...
    payed_bill_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("bill.id", ondelete="SET NULL"))
    merchant_category_code: Mapped[Optional[str]] = mapped_column(String(length=255))
    txn_date: Mapped[date] = mapped_column(Date, nullable=False)
    external_merchant_id: Mapped[Optional[str]] = mapped_column(String(length=255))
    custom_merchant_name: Mapped[Optional[str]] = mapped_column(String(length=255))
    original_merchant_name: Mapped[Optional[str]] = mapped_column(String(length=255))
    merchant_url: Mapped[Optional[str]] = mapped_column(Text)
    custom_note: Mapped[Optional[str]] = mapped_column(String(length=4096))
    original_note: Mapped[Optional[str]] = mapped_column(String(length=4096))
    ...
    txn_hash: Mapped[str] = mapped_column(CHAR(length=64), nullable=False)
    daycount: Mapped[int] = mapped_column(Integer, nullable=False)
...
class Budget(Base):
    ...
    created_dt: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=text("CURRENT_TIMESTAMP"))
    updated_dt: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=text("CURRENT_TIMESTAMP"))
    timespan: Mapped["TimeSpan"] = relationship(back_populates="budgets", lazy="noload", info=dto_field("private"))
    category: Mapped["Category"] = relationship(back_populates="budgets", lazy="joined")
